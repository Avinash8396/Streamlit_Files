    ...
    try:
        ...
        queries AS (
            SELECT
                ...
                TO_DATE(LEFT(START_TIME, 10), 'YYYY-MM-DD') as DATE,
                DAYNAME(TO_DATE(qh.START_TIME)) as day
        ...
        DATE,
        Day,
        SUM(query_cost) as COST_PER_DAY
        FROM queries2
        Group by all
        ;
        """
        ...
        # Optionally, you can also create a bubble chart
        fig = px.scatter(df, x="DATE", y="COST_PER_DAY",
                         title="Bubble Chart - Query Cost and Count",
                         labels={"DATE": "DATE", "COST_PER_DAY": "COST_PER_DAY (last 30d)"},
                         size_max=30)
        st.plotly_chart(fig)
